...
def assign_track_ids(people, tracker_state, max_match_distance=180.0, max_missed_frames=24):
    """Assign stable track IDs with persistence for short occlusions/out-of-frame cases."""
    ...
    if not tracks and tracker_state.get("previous_centers"):
        ...
        tracks = {track_id: {"center": center, "missed": 0} for track_id, center in legacy_centers.items()}

    detection_centers = []
    for person in people:
        detection_centers.append(_compute_person_center(person))

    # Build all candidate matches and solve greedily by shortest distance first.
    candidates = []
    ...
    for det_idx, person in enumerate(people):
        ...
        if assigned_track_id is None:
            ...
            next_track_id += 1
            tracks[assigned_track_id] = {"center": center, "missed": 0}

        person["track_id"] = int(assigned_track_id)
        if center is not None:
            tracks[assigned_track_id]["center"] = center

    # Keep unmatched tracks alive for a few frames to avoid ID switches.
    active_tracks = {}
    for track_id, track_data in tracks.items():
